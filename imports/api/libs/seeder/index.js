...
import getRandomUserId from '/tests/helpers/getRandomUserId';

class Seeder {
  constructor(collection, options) {
    if (!collection) this.throwSeederError('Please supply a MongoDB collection instance to seed.');
    if (!this.isValidMongoDBCollection(collection))
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      if (typeof value === 'object' && !Array.isArray(value)) {
        if (value.__crossreference) {
          ...
          processedData[key] = preProcessData(value);
        }
      } else {
  ...
  createDataItem(dataItem) {
    ...
    if (this.isUsersCollection()) {
      ...
      idOfItemCreated = this.collection.insert(preProcessData(dataItem));
    }
