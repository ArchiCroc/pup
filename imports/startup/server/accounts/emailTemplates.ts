...
import getPrivateFile from '/imports/api/libs/utils/getPrivateFile';
import templateToHtml from '/imports/api/libs/utils/handlebarsEmailToHtml';
import templateToText from '/imports/api/libs/utils/handlebarsEmailToText';
...
emailTemplates.resetPassword = {
  ...
  html(user, url) {
    if (!user?.emails || !user.emails[0].address) {
      throw Error('[emailTemaplates] Missing Email Address');
    }
